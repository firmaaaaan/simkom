...
@section('title', 'Kartu Kendali - ' . $komputer->nama_komputer)

@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0">Kartu Kendali - {{ $komputer->nama_komputer }}</h4>
            <a href="{{ route('admin.komputer.show', $komputer) }}" class="btn btn-outline-secondary">
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.komputer.kartu-kendali.store', $komputer) }}" method="POST">
                            @csrf
                            <div class="row g-3">
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    <label class="form-label">Detail Pemeriksaan</label>
                                        ...
                                        <div class="card-body p-0">
                                            <div class="table-responsive">
                                                <table class="table table-bordered mb-0">
                                                    ...
                                                    <tbody>
                                                        @forelse($komputer->hardware as $hw)
                                                            <tr>
                                                                <td>{{ $hw->nama_hardware }} ({{ $hw->kategori }})</td>
                                                                <td>
                                                                    ...
                                                                    <input type="hidden" name="items[{{ $loop->index }}][nama]" value="{{ $hw->nama_hardware }}">
                                                                </td>
                                                                <td>
                                                        ...
                                                        @empty
                                                            <tr>
                                                                <td colspan="3" class="text-center text-muted">Tidak ada hardware</td>
                                                            </tr>
                                                        @endforelse
                                                    </tbody>
                                                </table>
                                        ...
                                        <div class="card-body p-0">
                                            <div class="table-responsive">
                                                <table class="table table-bordered mb-0">
                                                    ...
                                                    <tbody>
                                                        @php $softwareIndex = 0; @endphp
                                                        @forelse($komputer->software as $sw)
                                                            @php $index = $komputer->hardware->count() + $softwareIndex; @endphp
                                                            <tr>
                                                                <td>{{ $sw->nama_software }} ({{ $sw->kategori }})</td>
                                                                <td>
                                                                    <select name="items[{{ $index }}][kondisi]" class="form-select form-select-sm">
                                                                        <option value="baik" {{ old("items.{$index}.kondisi") == 'baik' ? 'selected' : '' }}>Baik</option>
                                                                        <option value="cukup" {{ old("items.{$index}.kondisi") == 'cukup' ? 'selected' : '' }}>Cukup</option>
                                                                        <option value="rusak" {{ old("items.{$index}.kondisi") == 'rusak' ? 'selected' : '' }}>Rusak</option>
                                                                    </select>
                                                                    <input type="hidden" name="items[{{ $index }}][nama]" value="{{ $sw->nama_software }}">
                                                                </td>
                                                                <td>
                                                                    <input type="text" name="items[{{ $index }}][catatan]" class="form-control form-control-sm" value="{{ old("items.{$index}.catatan") }}" placeholder="Catatan...">
                                                                </td>
                                                            </tr>
                                                            @php $softwareIndex++; @endphp
                                                        @empty
                                                            <tr>
                                                                <td colspan="3" class="text-center text-muted">Tidak ada software</td>
                                                            </tr>
                                                        @endforelse
                                                    </tbody>
                                                </table>
                                ...
                                <div class="col-12 pt-2">
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('admin.komputer.show', $komputer) }}" class="btn btn-outline-secondary">Batal</a>
