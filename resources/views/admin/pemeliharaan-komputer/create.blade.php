...
@section('title', 'Tambah Pemeliharaan Komputer')

@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0">Tambah Pemeliharaan Komputer</h4>
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Form Pemeliharaan</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.pemeliharaan-komputer.store') }}" method="POST">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="laboratorium_id" id="laboratorium_id" class="form-select @error('laboratorium_id') is-invalid @enderror" required>
                                ...
                                @foreach($laboratoriums as $lab)
                                    <option value="{{ $lab->id }}" {{ old('laboratorium_id') == $lab->id ? 'selected' : '' }}>{{ $lab->nama_laboratorium }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="komputer_id" id="komputer_id" class="form-select @error('komputer_id') is-invalid @enderror" required>
                                ...
                                @foreach($komputers as $komp)
                                    <option value="{{ $komp->id }}" data-lab="{{ $komp->laboratorium_id }}" {{ old('komputer_id') == $komp->id ? 'selected' : '' }}>{{ $komp->nama_komputer }}</option>
                                @endforeach
                            </select>
                            @error('komputer_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" name="tanggal_pemeliharaan" id="tanggal_pemeliharaan" value="{{ old('tanggal_pemeliharaan', date('Y-m-d')) }}" class="form-control @error('tanggal_pemeliharaan') is-invalid @enderror" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="jenis_pemeliharaan" id="jenis_pemeliharaan" class="form-select @error('jenis_pemeliharaan') is-invalid @enderror" required>
                                <option value="preventif" {{ old('jenis_pemeliharaan') == 'preventif' ? 'selected' : '' }}>Preventif</option>
                                <option value="korektif" {{ old('jenis_pemeliharaan') == 'korektif' ? 'selected' : '' }}>Korektif</option>
                                <option value="upgrade" {{ old('jenis_pemeliharaan') == 'upgrade' ? 'selected' : '' }}>Upgrade</option>
                                <option value="penggantian" {{ old('jenis_pemeliharaan') == 'penggantian' ? 'selected' : '' }}>Penggantian</option>
                                <option value="lainnya" {{ old('jenis_pemeliharaan') == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="biaya" id="biaya" value="{{ old('biaya') }}" class="form-control @error('biaya') is-invalid @enderror" placeholder="0" min="0" step="1000">
                            @error('biaya')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="text" name="pic" id="pic" value="{{ old('pic') }}" class="form-control @error('pic') is-invalid @enderror" placeholder="Nama PIC">
                            @error('pic')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4" class="form-control @error('deskripsi') is-invalid @enderror" placeholder="Jelaskan detail pemeliharaan yang dilakukan..." required>{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
