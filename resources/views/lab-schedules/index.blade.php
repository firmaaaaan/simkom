                                    ...
                                    @if($schedule)
                                        <td class="group border border-gray-200 bg-green-50/40 text-left p-2.5 align-top cursor-grab active:cursor-grabbing" id="cell-{{ $cellKey }}"
                                            ...
                                            <div class="flex gap-1 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onclick="editSchedule({{ $schedule->id }})" class="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-500 text-white text-[10px] font-medium rounded hover:bg-blue-600 transition-colors">
                                                ...
                                                <button onclick="deleteSchedule({{ $schedule->id }}, @js($schedule->course_name.' ('.$schedule->time_label.')'))" class="inline-flex items-center gap-1 px-2 py-0.5 bg-red-500 text-white text-[10px] font-medium rounded hover:bg-red-600 transition-colors">
...
<script>
    ...
    function openCellModal(td) {
        ...
        document.getElementById('modal-subtitle').textContent = dayLabel + ' | ' + startTime + ' - ' + endTime + ' | ' + labCode + ' - ' + labName;
