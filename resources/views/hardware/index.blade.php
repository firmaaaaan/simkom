...
@section('header', 'Data Hardware')

@section('content')
<div x-data="{ selected: [] }">
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <p class="text-sm text-gray-500">Kelola seluruh data hardware laboratorium</p>
    <div class="flex items-center gap-2">
        <button x-show="selected.length > 0" x-cloak @click="if(confirm('Yakin ingin menghapus ' + selected.length + ' hardware?')) $refs.bulkForm.submit()"
        ...
        <a href="{{ route('hardware.import') }}" class="inline-flex items-center gap-2 px-4 py-2 border border-blue-600 text-blue-600 text-sm font-medium rounded-lg hover:bg-blue-50 transition-colors">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
            </svg>
            Import
        </a>
        <a href="{{ route('hardware.export') }}" class="inline-flex items-center gap-2 px-4 py-2 border border-purple-600 text-purple-600 text-sm font-medium rounded-lg hover:bg-purple-50 transition-colors">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
            Export
        </a>
        <a href="{{ route('hardware.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h2 class="text-base font-semibold text-gray-800">Daftar Hardware</h2>
        <form action="{{ route('hardware.index') }}" method="GET" class="relative">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
            </svg>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari hardware..." class="pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent w-full sm:w-64">
        </form>
    </div>
                ...
                <tr class="border-b border-gray-100">
                    <th class="text-left px-6 py-3 font-medium text-gray-500 w-10">
                        <input type="checkbox" class="rounded border-gray-300 text-green-600 focus:ring-green-500"
                            @change="selected = $event.target.checked ? @js($hardware->pluck('id')->toArray()) : []">
                    </th>
            ...
            <tbody>
                @forelse($hardware as $index => $item)
                    <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        ...
                        <td data-label="No" class="px-6 py-4 text-gray-500">{{ $hardware->firstItem() + $index }}</td>
                        <td data-label="Kode" class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-gray-100 text-gray-700">{{ $item->code }}</span>
                        </td>
                        <td data-label="Nama" class="px-6 py-4 font-medium text-gray-800">{{ $item->name }}</td>
                        ...
                        <td data-label="Aksi" class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('hardware.edit', $item) }}"
                                ...
                                <form action="{{ route('hardware.destroy', $item) }}" method="POST"
                                    onsubmit="return confirm('Yakin ingin menghapus hardware ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="9" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-500">Tidak ada data hardware</p>
                                    <p class="text-xs text-gray-400 mt-1">Silakan tambahkan hardware baru</p>
                                </div>
                            </div>
                        </td>
                    </tr>
...
<form x-ref="bulkForm" action="{{ route('hardware.bulk-destroy') }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
