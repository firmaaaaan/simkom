...
@section('header', 'Detail Hardware')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-bold text-gray-800">Detail Hardware</h2>
            <div class="flex items-center gap-2">
                <a href="{{ route('hardware.edit', $hardware) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <form action="{{ route('hardware.destroy', $hardware) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus hardware ini?')">
                    @csrf
                    @method('DELETE')
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-800 font-medium">{{ $hardware->name }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Kategori</p>
                    <p class="text-sm text-gray-800">{{ $hardware->category }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Merk</p>
                    <p class="text-sm text-gray-800">{{ $hardware->brand ?? '-' }}</p>
                </div>
                <div>
            ...
            @if($hardware->description)
                <div class="pt-4 border-t border-gray-100">
                    ...
                    <p class="text-sm text-gray-800">{{ $hardware->description }}</p>
                </div>
            @endif
        </div>
    </div>
