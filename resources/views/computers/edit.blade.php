...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-6">Form Edit Komputer</h2>
            ...
            <div class="space-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $computer->code) }}" required
                    ...
                    <div>
                        ...
                        <select name="laboratory_id" id="laboratory_id"
                            ...
                            @foreach($laboratories as $lab)
                                <option value="{{ $lab->id }}" {{ old('laboratory_id', $computer->laboratory_id) == $lab->id ? 'selected' : '' }}>{{ $lab->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="Tidak Aktif" {{ old('status', $computer->status) === 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        @forelse($hardware as $item)
                            <label class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                                <input type="checkbox" name="hardware[]" value="{{ $item->id }}" {{ in_array($item->id, old('hardware', $selectedHardware)) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        @forelse($software as $item)
                            <label class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                                <input type="checkbox" name="software[]" value="{{ $item->id }}" {{ in_array($item->id, old('software', $selectedSoftware)) ? 'checked' : '' }}
                                    class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500">
                                <span class="text-sm text-gray-700 truncate">{{ $item->name }}</span>
                            </label>
                        @empty
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        ...
                        placeholder="Keterangan tambahan tentang komputer...">{{ old('description', $computer->description) }}</textarea>
                </div>
            </div>
