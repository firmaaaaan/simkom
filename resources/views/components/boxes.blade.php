                            ...
                            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                ...
                                <td data-label="Aksi" class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <button type="button" data-box-id="{{ $box->id }}" class="js-view-box p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Lihat Detail">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            </svg>
                                        </button>
                                        <button type="button" data-box-code="{{ $box->code }}" data-box-name="{{ $box->name }}" class="js-show-qr p-1.5 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors" title="QR Code">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        ...
                                        <button type="button" data-box-id="{{ $box->id }}" data-box-name="{{ $box->name }}" data-box-location="{{ $box->location ?? '' }}" data-box-notes="{{ $box->notes ?? '' }}" class="js-edit-box p-1.5 text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Edit Box">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                        </button>
                                        <button type="button" data-box-id="{{ $box->id }}" data-box-code="{{ $box->code }}" class="js-delete-box p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<script>
    ...
    bindIf('bulkAddForm', 'submit', async function(e) {
        e.preventDefault();
    ...
    bindIf('generateForm', 'submit', async function(e) {
        e.preventDefault();
    ...
    bindIf('addComponentForm', 'submit', async function(e) {
        e.preventDefault();
    ...
    bindIf('generateModal', 'click', function(e) {
        if (e.target === this) closeGenerateModal();
    });

    bindIf('bulkAddModal', 'click', function(e) {
        if (e.target === this) closeBulkAddModal();
    });

    bindIf('boxDetailModal', 'click', function(e) {
        if (e.target === this) closeBoxDetailModal();
    });

    bindIf('qrModal', 'click', function(e) {
        if (e.target === this) closeQRModal();
    });

    bindIf('editBoxModal', 'click', function(e) {
    ...
    document.querySelectorAll('.js-view-box').forEach(btn => {
        btn.addEventListener('click', () => viewBox(btn.dataset.boxId));
    });

    document.querySelectorAll('.js-show-qr').forEach(btn => {
        btn.addEventListener('click', () => showQR(btn.dataset.boxCode, btn.dataset.boxName));
    });

    document.querySelectorAll('.js-edit-box').forEach(btn => {
        btn.addEventListener('click', () => openEditBoxModal(
            btn.dataset.boxId,
            btn.dataset.boxName,
            btn.dataset.boxLocation,
            btn.dataset.boxNotes
        ));
    });

    document.querySelectorAll('.js-delete-box').forEach(btn => {
        btn.addEventListener('click', () => deleteBox(btn.dataset.boxId, btn.dataset.boxCode));
    });

    bindIf('editBoxForm', 'submit', async function(e) {
        e.preventDefault();
