...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-1">Form Edit Komponen</h3>
        <p class="text-sm text-gray-500 mb-6">Perbarui data komponen</p>

        <form action="{{ route('components.update', $component) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $component->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $component->code) }}" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                        ...
                        <option value="Tersedia" {{ old('status', $component->status) === 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
                        <option value="Digunakan" {{ old('status', $component->status) === 'Digunakan' ? 'selected' : '' }}>Digunakan</option>
                        <option value="Rusak" {{ old('status', $component->status) === 'Rusak' ? 'selected' : '' }}>Rusak</option>
                        <option value="Maintenance" {{ old('status', $component->status) === 'Maintenance' ? 'selected' : '' }}>Maintenance</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="model" value="{{ old('model', $component->model) }}"
            ...
            <div>
                ...
                <input type="number" name="quantity" value="{{ old('quantity', $component->quantity) }}" required min="0"
                    class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
            </div>
            ...
            <div>
                ...
                @if($component->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $component->image) }}" alt="{{ $component->name }}" class="w-32 h-32 object-cover rounded-lg border border-gray-200">
                    </div>
                @endif
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Deskripsi komponen..."
                    class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 resize-none">{{ old('description', $component->description) }}</textarea>
            </div>
